...
interface PreferencesDrawerProps {
  ...
  visitorPhone: string;
  slug: string;
  onSuccess?: (matchingProperties: any[]) => void;
}
...
export default function PreferencesDrawer({
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`/api/landing-pages/${slug}/complete`, {
        method: "POST",
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || "فشل في حفظ التفضيلات");
      }
      ...
      toast({
        ...
        description: `تم العثور على ${data.matchingProperties?.length || 0} عقار مطابق`,
      });

      onSuccess?.(data.matchingProperties || []);
      onOpenChange(false);
    } catch (error: any) {
      toast({
        title: "خطأ",
    ...
    <Sheet open={open} onOpenChange={onOpenChange} dir="rtl">
      <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
        <SheetHeader>
          ...
          <SheetDescription>
            ساعدنا في العثور على أفضل العقارات المناسبة لك
          </SheetDescription>
        </SheetHeader>
